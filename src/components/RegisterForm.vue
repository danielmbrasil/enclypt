...
const validationSchema = toTypedSchema(
  zod
    .object({
      ...
      confirmMasterPassword: zod.string(),
    })
  ...
  <div class="flex min-h-svh flex-col items-center justify-center bg-surface-950 px-6 py-12">
    <div class="w-full max-w-sm">
      <div class="mb-10 text-center">
        ...
        <p class="mt-1 text-sm text-surface-500">Create your vault</p>
      </div>

      <Form :validation-schema="validationSchema" @submit="handleSubmit" class="space-y-5">
        <div>
          <label for="register-email" class="mb-1.5 block text-sm font-medium text-surface-300">
            Email
          </label>
        ...
        <div>
          <label
            for="register-master-password"
            class="mb-1.5 block text-sm font-medium text-surface-300"
          >
            Master password
          </label>
        ...
        <div>
          ...
          <Field name="confirmMasterPassword" v-slot="{ field, errors, meta }">
            <input
              v-bind="field"
          ...
          <ErrorMessage name="confirmMasterPassword" as="p" class="mt-1.5 text-xs text-red-400" />
        </div>
